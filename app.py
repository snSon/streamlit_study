...
try:
    ...
    if response.status_code == 200:
        ...
        if st.button("지역별 무더위 쉼터 갯수"):
            ...
            bars = plt.bar(list(shelter_counts.keys()), list(shelter_counts.values()), color=['red', 'dodgerblue'])
        ...
        if st.button("지역별 에어컨 있는 쉼터 갯수"):
            ...
            fig, ax = plt.subplots(figsize=(8, 6))
            bars = plt.bar(list(shelter_good.keys()), list(shelter_good.values()), color=['red', 'dodgerblue'])
            plt.xlabel("지역", fontproperties=font_prop)
            plt.ylabel("수", fontproperties=font_prop)
